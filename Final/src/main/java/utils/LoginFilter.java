...
@WebFilter(
		urlPatterns = { "/*" }, 
		initParams = { 
				@WebInitParam(name = "mustLogin1", value = "/backstageIndex.jsp"), 
...
public class LoginFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (request instanceof HttpServletRequest
			...
			if (servletPath.equals("/backstageIndex.jsp")) {           
				if (checkLogin(req)) {   
					//  需要登入，但已經登入
					...
					resp.sendRedirect(contextPath + "/backstageLogin.jsp");
					return;
				}
	...
	private boolean checkLogin(HttpServletRequest req) {
		...
		Object loginToken =  session.getAttribute("member");
		if (loginToken == null) {
			return false;
		} else {
			return true;
